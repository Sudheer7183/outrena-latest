...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):  # type: ignore[no-untyped-def]
        ...
        Two independent flags control what runs:

          SCHEDULER_ENABLED=True   → email-sending tick fires every
                                     SCHEDULER_TICK_SECONDS seconds.
                                     Set False to pause sending without
                                     touching reply/bounce polling.

          REPLY_POLLER_ENABLED=True → MailBridge is polled for inbound
                                      replies and bounces every
                                      MAILBRIDGE_REPLY_POLL_SECONDS.
                                      Keep True even when SCHEDULER_ENABLED
                                      is False so reply tracking stays live.

        The APScheduler instance is shared — it only starts when at least
        one of the two flags is True. Each job is registered independently.
        """
        _need_scheduler = settings.SCHEDULER_ENABLED or settings.REPLY_POLLER_ENABLED
        ...
        if _need_scheduler:
            ...
            scheduler = get_scheduler(
                email_tick_enabled=settings.SCHEDULER_ENABLED,
                reply_poller_enabled=settings.REPLY_POLLER_ENABLED,
            )
            try:
                scheduler.start()
                app.state.scheduler = scheduler
                ...
                _log.info(
                    "scheduler.started",
                    email_tick=settings.SCHEDULER_ENABLED,
                    reply_poller=settings.REPLY_POLLER_ENABLED,
                )
            except Exception:  # noqa: BLE001 — startup must not crash
                pass
        try:
            ...
            # PostHog: flush + shutdown FIRST
            posthog.shutdown()
            if _need_scheduler and getattr(app.state, "scheduler", None) is not None:
                try:
                    app.state.scheduler.shutdown(wait=False)
                except Exception:  # noqa: BLE001
                    pass
