...
"""
platform.py — SUPER_ADMIN platform-level API router.

Mounted at /api/platform in main.py (prefix="/api").
Exempt from TenantMiddleware via EXEMPT_PREFIXES in tenant_middleware.py.

Endpoints:
  POST   /platform/tenants                  — direct tenant provisioning
  GET    /platform/tenants                  — list all tenants with metrics
  GET    /platform/tenants/{slug}           — single tenant detail
  POST   /platform/tenants/{slug}/suspend   — suspend a tenant
  POST   /platform/tenants/{slug}/reactivate — reactivate a tenant
  GET    /platform/admin/tenants            — alias for GET /platform/tenants
  POST   /platform/admin/signups/{id}/approve — approve a signup request
  POST   /platform/admin/signups/{id}/reject  — reject a signup request
  GET    /platform/admin/signups            — list pending signup requests
  GET    /platform/admin/metrics            — platform-level metrics
  GET    /platform/admin/audit-logs         — cross-tenant audit log
"""
from __future__ import annotations
...
async def _require_super_admin(
    ...
    """Gate every platform endpoint to SUPER_ADMIN only."""
    verify_role(Role.SUPER_ADMIN, token)
    return token


# ---------------------------------------------------------------------------
# Tenant provisioning — POST /platform/tenants
# ---------------------------------------------------------------------------


class TenantCreateRequestExtended(TenantCreateRequest):
    """Extended create request that allows MFA to be skipped.

    skip_mfa=True omits the CONFIGURE_TOTP Keycloak requiredAction so the
    newly created tenant admin can log in immediately with their password.
    Use this when the super admin sets a temporary_password directly.

    skip_mfa=False (default) keeps the CONFIGURE_TOTP requirement, which is
    the NFR-015 compliant path for invitation-based provisioning.
    """
    skip_mfa: bool = False
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    summary="Provision a new tenant (SUPER_ADMIN)",
    tags=["platform"],
)
async def create_tenant(
    body: TenantCreateRequestExtended,
    db: AsyncSession = Depends(get_db),
    _token: TokenPayload = Depends(_require_super_admin),
) -> TenantCreatedResponse:
    """
    Full 6-step tenant provisioning triggered directly by a SUPER_ADMIN.

    When ``temporary_password`` is provided and ``send_invitation`` is False,
    ``skip_mfa`` defaults to True so the tenant admin can log in immediately
    without setting up an authenticator app. Pass ``skip_mfa=false`` explicitly
    to enforce TOTP setup even for direct-password provisioning.
    """
    settings = get_settings()

    # Convenience: if caller set a password and didn't ask for an invitation,
    # auto-enable skip_mfa unless they explicitly set it to False.
    effective_skip_mfa = body.skip_mfa
    if body.temporary_password and not body.send_invitation and not body.skip_mfa:
        # The caller provided a password and no invitation — they want the
        # admin to log in directly. Default skip_mfa to True.
        effective_skip_mfa = True
    ...
    try:
        ...
        logger.error("platform.create_tenant.failed", slug=body.slug, error=str(exc))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Tenant provisioning failed: {exc}",
        ) from exc

    base_domain = getattr(settings, "BASE_DOMAIN", "outrena.com") or "outrena.com"
    return TenantCreatedResponse(
        slug=slug,
        status="ACTIVE",
...
# ---------------------------------------------------------------------------
# Tenant listing — GET /platform/tenants  +  GET /platform/admin/tenants
# ---------------------------------------------------------------------------
...
@router.get(
    ...
    summary="List all tenants — admin alias (SUPER_ADMIN)",
    tags=["platform"],
)
async def list_tenants(
    db: AsyncSession = Depends(get_db),
    ...
    """Return all tenants with basic metrics from the public registry."""
    result = await db.execute(
        text(
            "SELECT tenant_id, slug, name, status, tenant_type, created_at "
            "FROM public.tenants "
...
# ---------------------------------------------------------------------------
# Tenant actions — suspend / reactivate
# ---------------------------------------------------------------------------


@router.post(
    "/platform/admin/tenants/{slug}/suspend",
...
@router.post(
    "/platform/admin/tenants/{slug}/reactivate",
...
# ---------------------------------------------------------------------------
# Signup approvals — /platform/admin/signups/*
# ---------------------------------------------------------------------------


@router.get(
    "/platform/admin/signups",
    summary="List signup requests (SUPER_ADMIN)",
    tags=["platform"],
)
async def list_signups(
    ...
    return await svc.list_signups(db=db, status_filter=status_filter)


@router.post(
    "/platform/admin/signups/{signup_id}/approve",
    summary="Approve a signup request and provision the tenant (SUPER_ADMIN)",
    tags=["platform"],
)
async def approve_signup(
    signup_id: str,
    db: AsyncSession = Depends(get_db),
    token: TokenPayload = Depends(_require_super_admin),
) -> dict:
    svc = PlatformAdminService()
    try:
        sid = int(signup_id)
    except ValueError:
        raise HTTPException(status_code=400, detail="signup_id must be an integer.")
    return await svc.approve_signup(
        signup_id=sid,
        reviewer_sub=token.sub,
        db=db,
    )


@router.post(
    "/platform/admin/signups/{signup_id}/reject",
    summary="Reject a signup request (SUPER_ADMIN)",
    tags=["platform"],
)
async def reject_signup(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="signup_id must be an integer.")
    return await svc.reject_signup(
        signup_id=sid,
        reviewer_sub=token.sub,
...
# ---------------------------------------------------------------------------
# Platform metrics — GET /platform/admin/metrics
# ---------------------------------------------------------------------------


@router.get(
    "/platform/admin/metrics",
    summary="Platform-level metrics (SUPER_ADMIN)",
    tags=["platform"],
)
async def platform_metrics(
    ...
    svc = PlatformAdminService()
    return await svc.platform_metrics(db=db)


# ---------------------------------------------------------------------------
# Audit logs — GET /platform/admin/audit-logs
# ---------------------------------------------------------------------------


@router.get(
    "/platform/admin/audit-logs",
    summary="Cross-tenant audit log (SUPER_ADMIN)",
    tags=["platform"],
)
