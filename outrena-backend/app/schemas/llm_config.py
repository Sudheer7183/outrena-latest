...
class LlmConfigResponse(BaseModel):
    ...
    id: str  # CUID for tenant configs; str(int) for global configs
    provider: str
    display_name: str
    api_key: str | None = None  # masked, never the raw value
    base_url: str | None = None
...
class TestLlmRequest(BaseModel):
    ...
    config_id is str to support both:
      • per-tenant LlmConfig (CUID string PK)
      • public.global_llm_config (integer PK, passed as str)
    """
