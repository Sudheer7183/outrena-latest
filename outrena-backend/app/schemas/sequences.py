...
#     mode=manual  — renders the template body/subject for EVERY prospect linked
#                    to the given icpProfileId, creating one Draft Sequence row
#                    (Touch 1, Day 1) per prospect. No LLM call is made.
#                    Returns all rendered rows in sequences[].

#     mode=llm     — passes the template body as a seed_body hint to the LLM
#                    generate-sequences flow for each prospect under the ICP,
#                    writing 7 personalised touches per prospect.
#     """
#     campaignId: str
#     icpProfileId: str               # ← replaces prospectId; renders for ALL prospects under this ICP
#     templateId: str
...
#     message: str
#     # Maps sequence id → prospect display name for the frontend card header.
#     # e.g. {"seq-uuid-123": "Jane Smith · Acme Corp"}
#     prospectMap: dict[str, str] = {}
...
class TemplateSendRequest(BaseModel):
    ...
    senderName: str | None = None      # logged-in user's full name (first + last)
    senderCompany: str | None = None   # from profile.senderCompany
    emailSignature: str | None = None  # from profile.emailSignature — appended if template has no footer
    physicalAddress: str | None = None # from profile.physicalAddress — appended in CAN-SPAM footer
