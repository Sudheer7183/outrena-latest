...
class DashboardService:
    ...
    async def get(
        ...
        *,
        ...
        # Top 5 campaigns by reply rate — compute from Sequence table.
        # Load all campaigns then join aggregate reply/sent counts in Python
        # to avoid a complex subquery that varies by DB engine.
        campaigns_stmt = select(Campaign)
        if effective_user_id is not None:
            campaigns_stmt = campaigns_stmt.where(
                Campaign.owner_user_id == effective_user_id
        ...
        campaigns_result = await db.execute(campaigns_stmt)
        all_campaigns = list(campaigns_result.scalars().all())

        # Aggregate sent + replied per campaign from the Sequence table.
        if all_campaigns:
            ...
            sent_result = await db.execute(
                select(
                    ...
                    func.count(Sequence.id).filter(
                        Sequence.repliedAt.is_not(None)
                    ).label("total_replied"),
                )
                .where(
                    Sequence.campaignId.in_(camp_ids),
                    Sequence.sentAt.is_not(None),
                )
                .group_by(Sequence.campaignId)
            )
            metrics_by_camp: dict[str, dict[str, int]] = {
                row[0]: {"sent": int(row[1]), "replied": int(row[2])}
                for row in sent_result.fetchall()
            }
        else:
            metrics_by_camp = {}

        def _reply_rate(camp_id: str) -> float:
            m = metrics_by_camp.get(camp_id, {})
            sent = m.get("sent", 0)
            replied = m.get("replied", 0)
            return (replied / sent) if sent else 0.0

        # Sort descending by reply rate, take top 5.
        sorted_campaigns = sorted(all_campaigns, key=lambda c: _reply_rate(c.id), reverse=True)
        top_campaigns = [
            DashboardTopCampaign(
                id=c.id,
                name=c.name,
                status=c.status,
                replyRate=_reply_rate(c.id) if metrics_by_camp.get(c.id) else None,
            )
