...
# from app.models.campaign_models import Campaign, CampaignResult, Deal
...
class AnalyticsService:
    ...
    async def tracking_summary(
        ...
        Aggregates Sequence rows sent within the last `days` days.
        Returns counts and rates for sent / opened / replied / bounced,
        plus a breakdown of the top bounce reasons and a by-status count.

        No external calls to MailBridge — reads only local Sequence rows
        that have already been stamped by apply_tracking_event().
        """
        from app.models.campaign_models import Sequence
        ...
        # Top bounce reasons — count non-null bounceReason strings.
        ...
        by_status: dict[str, int] = dict(status_counter)
