...
#             (await db.execute(select(func.count(Campaign.id)))).scalar() or 0
#         )
#         # FIX: count campaigns with status == "active" (not Scheduled sequences)
...
#             or 0
#         )
#         # FIX: sentThisWeek — count sequences where sentAt is set (i.e. actually sent)
#         sent_this_week = int(
#             (
#                 await db.execute(
#                     select(func.count(Sequence.id)).where(
...
#                 await db.execute(
#                     select(func.count(Sequence.id)).where(
#                         Sequence.repliedAt.is_not(None),
#                         Sequence.repliedAt >= week_ago,
#                     )
#                 )
#             ).scalar()
...
#         # FIX: resolve email quota from domain warming schedule when a fully
#         # DNS-verified domain exists, else fall back to env var.
#         email_quota_daily = 0
#         email_quota_source = "env"
#         try:
#             from app.models.config_models import Domain as _Domain
#             domains_result = await db.execute(
#                 select(_Domain).where(_Domain.isActive.is_(True))
#             )
#             all_domains = list(domains_result.scalars().all())
#             # Prefer a fully-verified domain (SPF + DKIM + DMARC all passed).
#             verified_domains = [
#                 d for d in all_domains
#                 if d.spfStatus and d.dkimStatus and d.dmarcStatus
#             ]
#             if verified_domains:
#                 # Use the highest dailySendLimit among verified domains.
#                 email_quota_daily = max(d.dailySendLimit for d in verified_domains)
#                 email_quota_source = "domain_warming"
#             else:
#                 from app.core.config import get_settings as _get_settings
#                 email_quota_daily = int(
#                     getattr(_get_settings(), "DEFAULT_USER_DAILY_EMAIL_QUOTA", 100)
#                 )
#         except Exception:  # noqa: BLE001
#             from app.core.config import get_settings as _get_settings
#             email_quota_daily = int(
...
#             pipelineValue=pipeline_value,
#             averageReplyRate=avg_reply_rate,
#             emailQuotaDaily=email_quota_daily,
#             emailQuotaSource=email_quota_source,
#         )
...
class AnalyticsService:
    async def list_metrics(
        self, db: AsyncSession, campaign_id: str | None = None, *, days: int = 90
    ) -> list[CampaignMetricResponse]:
        """List campaign metrics aggregated from the Sequence table.
        ...
        # FIX: apply the date-range filter so the date-range selector
        # actually changes the Campaign Performance table data.
        date_cutoff = datetime.now(timezone.utc) - timedelta(days=days)
        stmt = select(Sequence).where(
            Sequence.sentAt.is_not(None),
            Sequence.sentAt >= date_cutoff,
        )
        if campaign_id:
            stmt = stmt.where(Sequence.campaignId == campaign_id)
        result = await db.execute(stmt)
    ...
    async def diagnose(
        ...
        bounce_rate = (total_bounced / total_sent) if total_sent else 0.0
        reply_rate = (total_replied / total_sent) if total_sent else 0.0
        # open_rate intentionally omitted — pixel-blocked by most clients, unreliable signal
        ...
        summary = (
            f"Delivery: {bounce_rate:.1%} bounce. "
            f"Reply: {reply_rate:.1%}. Pipeline: {deal_count} deals."
        )
        return DiagnoseResponse(
            campaignId=campaign_id,
