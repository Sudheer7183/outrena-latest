...
#     _: object = Depends(require_role(Role.REP)),
# ) -> TrackingSummaryResponse:
#     """Tenant-wide sent / replied / bounced summary for the Reply Inbox dashboard.

#     Aggregates Sequence rows sent in the last `days` days.
#     No MailBridge call — reads local Sequence table only.
#     """
#     return await _service.tracking_summary(db, days=days)
...
async def list_metrics(
    ...
    # DTOs directly (aggregated from Sequence rows) — no ORM model_validate
    # step needed. The router returns them as-is.
    return await _service.list_metrics(db, campaign_id, days=days)
