...
#     # ── Fetch all ACTIVE, subscribed prospects under this ICP ─────────────────
#     # Use raw SQL to guarantee correct boolean and string comparisons:
#     #   - deleted_at IS NULL            — not soft-deleted
#     #   - anonymized IS NOT TRUE        — PII not wiped (covers NULL + false)
#     #   - suppressed IS NOT TRUE        — not suppressed (covers NULL + false)
#     #   - consent_status <> 'withdrawn' — consent not withdrawn
#     # Using raw SQL avoids SQLAlchemy ORM operator edge-cases and asyncpg
#     # prepared-statement cache issues that can cause IS_(False) to be
#     # mis-compiled when the column has mixed NULL/false values in the DB.
#     from sqlalchemy import text as _t
#     _icp_id = body.icpProfileId
...
#         {"icp_id": _icp_id},
#     )
#     _active_ids = [r[0] for r in _raw.fetchall()]

#     if not _active_ids:
#         raise HTTPException(
#             status.HTTP_422_UNPROCESSABLE_ENTITY,
#             f"No active, subscribed prospects are linked to ICP '{getattr(icp_row, 'name', body.icpProfileId)}'. "
#             "Unsubscribed and deleted prospects are excluded. Go to Prospects and link active prospects to this ICP.",
#         )

#     prospect_rows = (
#         await db.execute(
#             select(_Prospect).where(_Prospect.id.in_(_active_ids))
#         )
#     ).scalars().all()
