...
#             # ── Multi-touch logic ─────────────────────────────────────────────
#             # Find ALL existing sequences for this (campaign, prospect) pair,
#             # ordered by touchNumber descending so we can read the highest touch.
...
#                         _SeqModel.prospectId == prospect.id,
#                     )
#                     .order_by(_SeqModel.touchNumber.desc())
#                 )
#             ).scalars().all()

#             # Determine next touch number and whether to edit or create.
#             #
#             # Rules (mirrors how AI-generated cadences work):
#             #   - No existing sequences → create Touch 1
#             #   - Latest touch is Draft/Scheduled (not yet sent) → OVERWRITE it
#             #     (user is editing the pending touch before it goes out)
#             #   - Latest touch is Sent/Bounced/Replied/Failed (already sent) →
#             #     CREATE a new touch with touchNumber = max + 1
#             #     This is the "Touch 2, Touch 3..." scenario.
#             #
#             # SEVEN_TOUCH_CADENCE defines the angle/sendDay for each touch number.
#             # For manual sends we use the same cadence so the touch metadata is
#             # consistent with AI-generated sequences.
#             from app.schemas.sequences import SEVEN_TOUCH_CADENCE as _CADENCE
#             _TERMINAL = {"Sent", "Replied", "Bounced", "Failed"}

#             _cadence_map = {c.touchNumber: c for c in _CADENCE}
#             _prose_label = lambda p: (
#                 f"{getattr(p, 'firstName', '') or ''} "
#                 f"{getattr(p, 'lastName', '') or ''}".strip()
#                 or p.id
#             )

#             if not existing_rows:
#                 # No sequences for this prospect in this campaign → Touch 1
#                 next_touch = 1
#                 edit_existing = None
#             else:
#                 latest = existing_rows[0]  # highest touchNumber (desc order)
#                 latest_status = (
#                     latest.status.value
#                     if hasattr(latest.status, "value")
#                     else str(latest.status)
#                 )
#                 if latest_status in _TERMINAL:
#                     # Latest touch already sent → start the next touch
#                     next_touch = latest.touchNumber + 1
#                     edit_existing = None
#                 else:
#                     # Latest touch still pending (Draft/Scheduled) → overwrite it
#                     next_touch = latest.touchNumber
#                     edit_existing = latest

#             # Cap at 7 touches (matches SEVEN_TOUCH_CADENCE length)
#             if next_touch > 7:
#                 skipped.append(_prose_label(prospect))
#                 continue

#             cadence = _cadence_map.get(next_touch)
#             touch_angle = cadence.angle.value if cadence else "FirstTouch"
#             touch_framework = (
#                 getattr(template, "framework", None)
#                 or (cadence.defaultFramework if cadence else "Template")
#             )
#             touch_send_day = cadence.sendDay if cadence else next_touch
...
#                 # Overwrite the existing pending touch with the new template content
#                 edit_existing.subjectLine  = rendered_subject
#                 edit_existing.bodyCopy     = rendered_body
#                 edit_existing.framework    = touch_framework
#                 edit_existing.angle        = touch_angle
#                 edit_existing.status       = EmailStatus.Draft
#                 edit_existing.owner_user_id = token.sub
#                 saved_id = edit_existing.id
#                 await db.commit()

#                 # Re-apply search_path after commit (asyncpg strips it on pool return)
#                 await db.execute(_sql_text(f'SET search_path TO "{_schema}", public'))
#                 seq_row = (
#                     await db.execute(
#                         select(_SeqModel)
...
#                     continue
#             else:
#                 # Create a new touch row
#                 seq = _SeqModel(
#                     id=new_id,
#                     campaignId=body.campaignId,
#                     prospectId=prospect.id,
#                     touchNumber=next_touch,
#                     sendDay=touch_send_day,
#                     channel="email",
#                     angle=touch_angle,
#                     framework=touch_framework,
#                     subjectLine=rendered_subject,
#                     bodyCopy=rendered_body,
#                     status=EmailStatus.Draft,
#                     owner_user_id=token.sub,
...
#                 # Re-apply search_path after commit
#                 await db.execute(_sql_text(f'SET search_path TO "{_schema}", public'))
#                 seq_row = (
#                     await db.execute(
#                         select(_SeqModel)
    ...
    if body.mode == "manual":
        ...
        for prospect in prospect_rows:
            ...
            if not existing_rows:
                ...
                # If ANY previous touch bounced, skip this prospect entirely.
                # Their email address does not exist — sending more touches
                # wastes quota, hurts deliverability, and inflates bounce counts.
                # Note: _stamp_bounce also sets Prospect.suppressed=True so future
                # template-send calls will exclude them at the prospect filter level.
                # This guard catches the race where the template-send runs before
                # the poller has had a chance to suppress the prospect.
                has_bounced = any(
                    (
