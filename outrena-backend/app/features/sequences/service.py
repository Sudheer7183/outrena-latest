...
# # #         # ── Suppression gate ──────────────────────────────────────────────────
# # #         # Check the prospect's suppressed flag before sending. If the prospect
# # #         # has unsubscribed (suppressed=True / consent_status='withdrawn'), we
# # #         # must not send regardless of who triggers the send (Send Now or
# # #         # Scheduler). The Scheduler already checks this; Send Now must too.
# # #         if prospect is not None and (
# # #             getattr(prospect, "suppressed", False)
# # #             or getattr(prospect, "consent_status", None) == "withdrawn"
# # #         ):
# # #             return SendEmailResponse(
# # #                 id=sequence_id,
# # #                 status=EmailStatus.Failed,
...
# # #         # The scheduler tick does this replacement; Send Now must also do it
# # #         # so the delivered email contains a working unsubscribe link.
...
# # #                     _scheme = "https"
# # #                     _real_unsub = (
# # #                         f"{_scheme}://{_base}/api/v1/public/unsubscribe"
# # #                         f"?token={_prospect_token}&tenant_slug={_tenant_slug}"
# # #                     )
# # #                     body_to_send = body_to_send.replace("{{unsubscribe_url}}", _real_unsub)
# # #             except Exception:  # noqa: BLE001 — never block send
# # #                 pass
...
# #         # The scheduler tick does this replacement; Send Now must also do it
# #         # so the delivered email contains a working unsubscribe link.
# #         # URL points to the React page /p/unsubscribe which then calls the
# #         # backend API — NOT directly to the API endpoint.
# #         body_to_send = seq.bodyCopy or ""
# #         if "{{unsubscribe_url}}" in body_to_send and prospect is not None:
# #             try:
# #                 from app.utils.tenant_context import resolve_tenant_slug as _rts
...
#         # ── Suppression gate ──────────────────────────────────────────────────
#         # Refuse to schedule emails for prospects who have unsubscribed.
#         # This prevents reps from manually re-scheduling after an unsubscribe.
#         from sqlalchemy import select
#         from app.models.prospect_models import Prospect as _Prospect
#         prospect_result = await db.execute(
#             select(_Prospect).where(_Prospect.id == seq.prospectId)
#         )
#         prospect = prospect_result.scalar_one_or_none()
#         if prospect is not None and (
#             getattr(prospect, "suppressed", False)
...
#                 status_code=422,
#                 detail=(
#                     f"Cannot schedule email — prospect has unsubscribed. "
#                     "Their consent status is 'withdrawn'."
#                 ),
#             )
