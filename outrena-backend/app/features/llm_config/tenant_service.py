...
def _settings_as_dict(raw: Any) -> dict:
    """
    Safely coerce the settings column value to a plain dict.

    The column is declared PG_JSON but existing rows may hold a raw JSON
    string (e.g. '{}') when written before SQLAlchemy's JSON coercion was
    active. Handles str, dict, and None defensively — never raises.
    """
    if raw is None:
        return {}
...
class TenantLlmConfigService:
    ...
    async def update(
        ...
        # FIX: use _settings_as_dict() — item.settings may be a JSON string
        # (not a dict) when the row was written before PG_JSON coercion was
        # active. Calling dict() on a string raises ValueError.
        if new_max_tokens is not None:
            settings = _settings_as_dict(item.settings)
            settings["max_tokens"] = new_max_tokens
            item.settings = settings
        if new_temperature is not None:
            settings = _settings_as_dict(item.settings)
        ...
        if new_api_key:
            try:
                ...
                logger.error(
                    "tenant_llm.update.encrypt_failed",
                    config_id=config_id,
                    error=str(exc),
                )
                raise
    ...
    async def test_llm(
        ...
        _KEY_OPTIONAL_PROVIDERS = ("local", "zai")

        if not config.apiKey and config.provider not in _KEY_OPTIONAL_PROVIDERS:
            return TestLlmResponse(
                ok=False, content="", error="No API key stored for this config."
            )

        # Decrypt the stored key, or use empty string for key-optional providers.
        if config.apiKey:
            try:
                from app.services.secret_service import decrypt_at_rest
                api_key = decrypt_at_rest(config.apiKey)
        ...
        # FIX: pass settings as a JSON string so _parse_settings() in
        # llm_service can json.loads() it — the service expects a string,
        # not a dict, for the legacy SimpleNamespace shim path.
        raw_settings = _settings_as_dict(config.settings)
        legacy_config = SimpleNamespace(
            provider=config.provider,
            name=config.name,
            modelId=config.modelId,
            ...
            isActive=config.isActive,
            isDefault=config.isDefault,
            settings=_json.dumps(raw_settings),
            global_llm_config_id=None,
        )
    ...
    def to_response(row: LlmConfig) -> LlmConfigResponse:
        ...
        settings = _settings_as_dict(row.settings)
