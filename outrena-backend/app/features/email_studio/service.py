...
FIX (personalisation): generate_email() now receives full sender context
(senderFirstName, senderLastName, senderTitle, senderCompany, senderOffer)
from the request and passes it to the LLM so the generated body contains
no [Your Name] / {{placeholder}} artefacts.

FIX (signature): After generation, the caller's senderSignature block is
appended verbatim to the body. Stored per-user in Keycloak attributes and
sent by the frontend from the current user's profile.

FIX (CAN-SPAM footer): An unsubscribe link ({{unsubscribe_url}} token,
replaced by MailBridge at send-time) and optional physicalAddress are
appended to every generated body so compliance_check() always passes the
can_spam_unsubscribe and can_spam_address rules.

FIX (LLM routing): All LLM-powered methods use call_llm(config, messages)
with the tenant's configured LlmConfig — NOT the legacy get_llm_service()
ZAI stub that returned [LLM-STUB] placeholders.
"""
from __future__ import annotations
...
def _build_can_spam_footer(unsubscribe_url: str, physical_address: str | None) -> str:
    """
    Build the plain-text CAN-SPAM footer appended to every outbound email.

    {{unsubscribe_url}} is replaced at send-time by MailBridge with a
    per-recipient one-click unsubscribe link.  The physical address is
    included when provided; tenants should configure it in their settings.
    """
    lines = [
        "",
        "---",
...
class EmailStudioService:
    ...
    async def generate_email(
        ...
        """
        Generate a fully personalised cold outreach email using the tenant's LLM.

        Personalisation changes vs. original:
        - Sender's full name, title, company, and value proposition are injected
          into the LLM prompt so it writes a real sign-off — no [Your Name].
        - Prospect's first name is used in the greeting — no {{first_name}}.
        - After generation the caller's signature block is appended verbatim.
        - A CAN-SPAM footer (unsubscribe link + physical address) is appended
          so compliance_check() passes without manual intervention.
        """
        # ── 1. Fetch prospect ────────────────────────────────────────────────
        prospect_result = await db.execute(
            select(Prospect).where(Prospect.id == body.prospectId)
        ...
        # ── 3. Build sender context strings ──────────────────────────────────
        # Resolve the sender's display name; fall back to "the sender" so the
        # LLM still produces a coherent email even if the caller omits the field.
        sender_name = " ".join(
            part for part in [body.senderFirstName, body.senderLastName] if part
        ).strip() or "the sender"

        sender_context_parts = []
        if body.senderTitle:
            sender_context_parts.append(body.senderTitle)
        if body.senderCompany:
            sender_context_parts.append(f"at {body.senderCompany}")
        sender_context = (
            f" ({', '.join(sender_context_parts)})" if sender_context_parts else ""
        )

        offer_line = (
            f" Our offer: {body.senderOffer}." if body.senderOffer else ""
        )
        ...
        system_msg = (
            "You are an expert cold-email copywriter. "
            "Respond ONLY with a valid JSON object — no markdown fences, no preamble. "
            "Never use placeholder text such as [Your Name], {{first_name}}, "
            "[Company], or any bracketed/handlebars tokens. "
            "Use only the actual names and values provided in the prompt."
        )

        prospect_first = (prospect.firstName or "there").strip()
        prospect_full = f"{prospect.firstName or ''} {prospect.lastName or ''}".strip()
        prospect_context = f"{prospect_full} ({prospect.title or 'unknown title'} at {prospect.company or 'their company'})"

        user_msg = (
            f"Write a cold outreach email TO: {prospect_context}. "
            f"Address them by their first name: {prospect_first}. "
            f"The email is FROM: {sender_name}{sender_context}.{offer_line} "
            f"Touch #{body.touchNumber}, angle: {body.angle}, "
            f"framework: {body.framework or 'Trigger-Based'}, "
            f"tone: {body.tone}, max length: {body.maxLength} words. "
            f"Sign the email with the sender's real name ({sender_name}) — "
            "do NOT write [Your Name] or any placeholder. "
            "Return JSON: "
            '{"subject": "...", "body": "...", "qaScore": 75, '
            '"qaDetails": {"spamminess": 10, "personalization": 80, "clarity": 85}, '
            '"personalisationConfidence": 0.85}'
        )
        ...
        # ── 7. Append sender signature ────────────────────────────────────────
        # The signature block is stored per-user in Keycloak (emailSignature
        # attribute) and sent by the frontend from the current user's profile.
        # It is appended verbatim after the LLM-generated body so each rep's
        # emails carry their own name, title, phone, and branding.
        if body.senderSignature and body_text:
            body_text = f"{body_text}\n\n{body.senderSignature}"

        # ── 8. Append CAN-SPAM footer ─────────────────────────────────────────
        # Appended unconditionally so that compliance_check() always passes the
        # can_spam_unsubscribe and can_spam_address rules without manual edits.
        # MailBridge replaces {{unsubscribe_url}} with a per-recipient link at
        # send time.
        if body_text:
            body_text += _build_can_spam_footer(
                body.unsubscribeUrl, body.physicalAddress
            )
