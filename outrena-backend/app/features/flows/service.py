...
# #                 ) from exc
# #             raise
# #         item = await db.get(FlowAbTest, item.id)
# #         return item
...
#         item_id = item.id  # capture before commit clears identity map
#         await db.commit()
#         # Re-fetch via SELECT — avoids search_path loss on re-pooled asyncpg connections.
#         item = (
#             await db.execute(select(ProspectingFlow).where(ProspectingFlow.id == item_id))
#         ).scalar_one()
#         return item
...
#         await db.commit()
#         flow = (
#             await db.execute(select(ProspectingFlow).where(ProspectingFlow.id == flow_id))
#         ).scalar_one()
#         return flow
...
#             triggeredBy=triggered_by,
#             triggeredById=triggered_by_id,
#             config=config or {},           # PG_JSON column — pass dict, not string
#             stats={},                       # PG_JSON column — pass dict, not string
#             importedProspectIds=[],         # PG_JSON column — pass list, not string
#             startedAt=datetime.now(timezone.utc),
#         )
#         db.add(run)
#         await db.flush()  # populate run.id
...
#         run.status = FlowRunStatus.COMPLETED
#         if stats is not None:
#             run.stats = stats               # PG_JSON column — assign dict directly
#         if imported_prospect_ids is not None:
#             run.importedProspectIds = imported_prospect_ids  # PG_JSON — assign list
#         run.completedAt = datetime.now(timezone.utc)
#         await db.flush()
...
#         run.errorMessage = error_message[:1000] if error_message else None
#         if stats is not None:
#             run.stats = stats               # PG_JSON — assign dict directly
#         run.completedAt = datetime.now(timezone.utc)
#         await db.flush()
...
#         await db.commit()
#         item = (
#             await db.execute(select(AutopilotQueue).where(AutopilotQueue.id == item_id))
#         ).scalar_one()
#         return item
...
#         item_id = item.id
#         item = (
#             await db.execute(select(FlowAbTest).where(FlowAbTest.id == item_id))
#         ).scalar_one()
#         return item
...
#         await db.commit()
#         item = (
#             await db.execute(select(FlowWebhook).where(FlowWebhook.id == item_id))
#         ).scalar_one()
#         return item
...
#         delivery = (
#             await db.execute(
#                 select(FlowWebhookDelivery).where(FlowWebhookDelivery.id == delivery_id)
#             )
#         ).scalar_one()
#         return delivery
...
class FlowRunService:
    ...
    async def get_run_with_steps(
        ...
        NOTE: We must NOT assign to run.steps via the ORM descriptor — that
        triggers a synchronous lazy-load (MissingGreenlet) in async sessions
        because SQLAlchemy reads the old value through the relationship before
        setting the new one. Instead we use selectinload to eager-load the
        relationship in a single query so the attribute is already populated
        when the router calls model_validate().
        """
