...
async def list_my_campaigns(
    ...
    """Return only the calling user's campaigns (always filtered by owner_user_id)."""
    items, total, counts = await _service.list_campaigns(
        db,
        status=campaign_status,
        limit=limit,
        offset=offset,
    ...
    from app.schemas.campaigns import CampaignCountSummary as _CCS
    def _r(c):
        resp = CampaignResponse.model_validate(c)
        cc = counts.get(c.id, {"prospects": 0, "sequences": 0, "collaterals": 0})
        resp.count = _CCS(prospects=cc["prospects"], sequences=cc["sequences"], collaterals=cc["collaterals"])
        return resp
    return CampaignListResponse(items=[_r(i) for i in items], total=total, limit=limit, offset=offset)
...
async def list_campaign_prospects(
    ...
    """Return all prospects linked to a campaign, enriched with prospect data.
 
    Primary source: CampaignProspect join table.
    Fallback: when the join table is empty (prospects were linked implicitly via
    Sequence generation rather than the explicit "Add Prospects" UI flow),
    derive distinct prospects from the Sequence.prospectId column instead.
    This ensures the Prospects tab always reflects reality regardless of which
    path was used to associate prospects with the campaign.
    """
    from sqlalchemy import select as _select
    from app.models.campaign_models import CampaignProspect, Sequence as _Seq
    from app.models.prospect_models import Prospect
 
    # ── Primary: CampaignProspect join table ──────────────────────────────
    cp_result = await db.execute(
        _select(CampaignProspect).where(CampaignProspect.campaignId == campaign_id)
    )
    links = cp_result.scalars().all()
 
    if links:
        # Fast path — join table is populated; N+1 is acceptable for typical
        # campaign sizes (< 1000 prospects). Bulk-fetch all prospects in one query.
        prospect_ids = [lnk.prospectId for lnk in links]
        prospects_result = await db.execute(
            _select(Prospect).where(Prospect.id.in_(prospect_ids))
        )
        prospect_map = {p.id: p for p in prospects_result.scalars().all()}
 
        rows = []
        for link in links:
            p = prospect_map.get(link.prospectId)
            rows.append({
                "id": link.id,
                "campaignId": link.campaignId,
                "prospectId": link.prospectId,
    ...
    # ── Fallback: derive prospects from Sequence.prospectId ───────────────
    # Sequences were generated directly against prospect rows; the CampaignProspect
    # join table was never written. Return one row per distinct prospect.
    seq_result = await db.execute(
        _select(_Seq.prospectId)
        .where(
            _Seq.campaignId == campaign_id,
    ...
    seq_prospect_ids = [r[0] for r in seq_result.all()]
 
    if not seq_prospect_ids:
        return []
 
    prospects_result = await db.execute(
        _select(Prospect).where(Prospect.id.in_(seq_prospect_ids))
    )
    prospects = prospects_result.scalars().all()
 
    return [
        {
            # Synthetic id — no CampaignProspect row exists.
            "id": f"seq-derived-{p.id}",
            "campaignId": campaign_id,
            "prospectId": p.id,
            "status": "active",
            "createdAt": None,
...
async def list_campaigns(
    ...
    def _build_response(c: "Campaign") -> "CampaignResponse":
        from app.schemas.campaigns import CampaignCountSummary
        resp = CampaignResponse.model_validate(c)
        c_counts = counts.get(c.id, {"prospects": 0, "sequences": 0, "collaterals": 0})
        resp.count = CampaignCountSummary(
            prospects=c_counts["prospects"],
            sequences=c_counts["sequences"],
            collaterals=c_counts["collaterals"],
        )
        return resp
