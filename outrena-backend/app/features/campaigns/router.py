...
# #     service.link_prospect commits the CampaignProspect row and then
# #     attempts (best-effort) sequence auto-generation. If sequence gen
# #     fails it rolls back only that sub-operation and leaves the session
# #     clean, so the fetch below is always safe.
# #     """
# #     await _service.link_prospect(db, body)
# #     # Expire all cached ORM state so we get a fresh SELECT, not a
# #     # stale identity-map hit from before the rollback/commit above.
# #     db.expire_all()
# #     item = await _service.get(db, body.campaignId)
# #     if item is None:  # defensive — should not happen after link
# #         raise HTTPException(status.HTTP_404_NOT_FOUND, "Campaign not found.")
...
#     campaign_id: str = Query(..., description="Campaign ID to fetch linked prospects for"),
#     db: AsyncSession = Depends(get_db),
#     _: TokenPayload = Depends(require_role(Role.REP)),
# ) -> list[dict]:
#     """Return all CampaignProspect rows for a given campaign, each enriched
#     with the prospect firstName, lastName, email, title, company, and seniority.
#     """
#     from sqlalchemy import select as _select
#     from app.models.campaign_models import CampaignProspect
...
#     _: TokenPayload = Depends(require_role(Role.REP)),
# ) -> dict:
#     """Link a prospect to a campaign.

#     Returns a minimal {added, prospectId, campaignId} dict — NOT a full
#     CampaignResponse — to avoid MissingGreenlet errors from lazy-loading
#     Campaign relationships (icpProfile, prospects, sequences) outside the
#     async context. The frontend refreshes the prospect list by invalidating
#     the campaign-prospects query after this call succeeds.

#     Role lowered to REP so individual contributors can manage their own
#     campaign-prospect links without needing MANAGER elevation.
#     """
#     await _service.link_prospect(db, body)
#     return {
#         "added": 1,
...
async def link_prospect(
    ...
    FIX 1: Role lowered from MANAGER → REP so campaign owners can link
    prospects without elevation.

    FIX 2: Returns a minimal dict instead of CampaignResponse.model_validate()
    to avoid MissingGreenlet from lazy-loading Campaign relationships outside
    the async context.

    FIX 3: Passes token.sub as owner_user_id so auto-generated Sequence rows
    are stamped with the real user ID (not "system"). Without this, the
    GET /sequences?campaign_id=X call filters by owner_user_id=token.sub and
    finds 0 rows because all sequences were stamped with owner_user_id="system".
    """
    from app.features.sequences.service import SequenceService

    await _service.link_prospect(db, body)

    # Re-run sequence auto-generation with the real owner_user_id.
    # The link_prospect service already tried this with owner_user_id=None
    # (falls back to "system"), but we need them stamped with token.sub
    # so the sequences are visible in the per-user list query.
    # auto_generate_for_campaign is idempotent — it skips existing rows.
    try:
        ...
        _sl.get_logger(__name__).warning(
            "link_prospect.sequence_restamp_failed",
            campaign_id=body.campaignId,
            prospect_id=body.prospectId,
            error=str(exc),
        )
...
async def generate_sequences(
    ...
    # FIX: restamp any existing sequences that were auto-generated with
    # owner_user_id="system" (created by link_prospect before the token
    # was passed through). Without this, GET /sequences?campaign_id=X
    # filters by owner_user_id=token.sub and returns 0 rows for those
    # sequences, making the Sequences tab appear empty.
    from app.models.campaign_models import Sequence as _Seq
    restamp_result = await db.execute(
        _select(_Seq).where(
    ...
    restamped = 0
    for seq in restamp_result.scalars().all():
    ...
    return {
        "message": (
            f"Generated {total_created} sequence rows for {len(prospect_ids)} prospect(s)."
            + (f" Restamped {restamped} existing sequences to your user ID." if restamped else "")
        ),
        "created": total_created,
        "restamped": restamped,
        "prospects": len(prospect_ids),
