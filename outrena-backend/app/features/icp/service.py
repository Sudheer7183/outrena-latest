...
class IcpService:
    ...
    async def list_profiles(
        ...
        """Return ICP profiles with a prospect count per profile.

        Uses a single LEFT JOIN / subquery so we avoid N+1 queries.
        The prospect count is injected as a transient attribute on the
        IcpResponse schema (not on the ORM model) so the ORM layer is
        not polluted.
        """
        # Subquery: count of prospects per icp_profile_id
        count_sq = (
            select(
                Prospect.icpProfileId.label("icp_id"),
                func.count(Prospect.id).label("cnt"),
            )
            .group_by(Prospect.icpProfileId)
            .subquery()
        )

        stmt = (
            select(IcpProfile, count_sq.c.cnt)
            .outerjoin(count_sq, IcpProfile.id == count_sq.c.icp_id)
            .offset(offset)
            .limit(limit)
        )
        rows = (await db.execute(stmt)).all()

        results: list[IcpResponse] = []
        for icp, cnt in rows:
            resp = IcpResponse.model_validate(icp)
            resp.prospectCount = int(cnt or 0)
            results.append(resp)
        return results
