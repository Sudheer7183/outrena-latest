...
class ProspectService:
    ...
    async def create(
        ...
        # Auto-derive timezone from email domain when not explicitly provided.
        # Uses country-code TLD mapping (e.g. .de → Europe/Berlin, .in → Asia/Kolkata).
        # Generic TLDs (.com, .io, .org) return None — scheduler sends without
        # business-hours restriction rather than guessing a wrong timezone.
        if not data.get("timezone"):
            email_val = data.get("email") or ""
            derived_tz = derive_timezone_from_email(email_val)
            if derived_tz:
                data["timezone"] = derived_tz
                logger.debug(
                    "prospect.create.timezone_derived",
                    email_domain=email_val.split("@", 1)[-1] if "@" in email_val else "",
                    timezone=derived_tz,
                )
