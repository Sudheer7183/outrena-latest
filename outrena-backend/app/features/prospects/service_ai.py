...
class ProspectAiService:
    ...
    async def _get_llm_config(
        ...
        """Resolve LLM config from public.global_llm_config (correct schema).

        The prospect AI service runs inside a tenant-scoped DB session whose
        search_path is set to the tenant schema.  GlobalLlmConfig lives in the
        public schema, so we open a *separate* AsyncSessionLocal and explicitly
        set search_path to public before querying.
        """
        async with AsyncSessionLocal() as pub_db:
            await pub_db.execute(
                __import__("sqlalchemy", fromlist=["text"]).text('SET search_path TO "public"')
            )
            ...
            row: GlobalLlmConfig | None = result.scalar_one_or_none()
            if row is None:
                return None
            raw_key: str = decrypt_at_rest(row.api_key_encrypted)
