...
#             c.status                                          AS campaign_status,
#             COUNT(s.id)                                                            AS total_sequences,
#             COUNT(s.id) FILTER (WHERE s.status IN ('Scheduled','QaPassed') AND s."touchNumber" <= 7) AS scheduled,
#             COUNT(s.id) FILTER (WHERE s.status = 'Sent')                               AS sent,
#             COUNT(s.id) FILTER (WHERE s.status NOT IN ('Scheduled','Sent','Replied','Bounced','Failed','Draft','QaFailed','QaPassed')) AS skipped,
#             COUNT(s.id) FILTER (WHERE s.status = 'Replied')                            AS replied,
#             COUNT(s.id) FILTER (WHERE s.status = 'Bounced')                            AS bounced,
#             COUNT(s.id) FILTER (WHERE s.status = 'Failed')                             AS failed,
#             MIN(s."createdAt") FILTER (WHERE s.status IN ('Scheduled','QaPassed') AND s."touchNumber" <= 7) AS next_send_at
#         FROM "Campaign" c
#         LEFT JOIN "Sequence" s ON s."campaignId" = c.id
#         WHERE {where_sql}
#         GROUP BY c.id, c.name, c.status
    ...
    *,
    ...
    data_sql = text(f"""
        SELECT
            ...
            c.status                                          AS campaign_status,
            COUNT(s.id)                                                            AS total_sequences,
            COUNT(s.id) FILTER (WHERE s.status = 'Scheduled' AND s."touchNumber" <= 7) AS scheduled,
            COUNT(s.id) FILTER (WHERE s.status = 'Sent')                               AS sent,
            COUNT(s.id) FILTER (WHERE s.status NOT IN ('Scheduled','Sent','Replied','Bounced','Failed','Draft','QaFailed','QaPassed')) AS skipped,
            COUNT(s.id) FILTER (WHERE s.status = 'Replied')                            AS replied,
            COUNT(s.id) FILTER (WHERE s.status = 'Bounced')                            AS bounced,
            COUNT(s.id) FILTER (WHERE s.status = 'Failed')                             AS failed,
            MIN(s."createdAt") FILTER (WHERE s.status = 'Scheduled' AND s."touchNumber" <= 7) AS next_send_at
        FROM "Campaign" c
        LEFT JOIN "Sequence" s ON s."campaignId" = c.id
        WHERE {where_sql}
        GROUP BY c.id, c.name, c.status
