# # # """
# # # scheduler_service.py — Phase 5 APScheduler tick + manual-tick + status.

# # # Phase 3 contract (preserved):
# # #   - get_status(db) → lazily seeds + returns the singleton SchedulerStatus row.
# # #   - manual_tick(db, tenant_scoped, max_send) → synchronous tick used by
# # #     POST /api/v1/scheduler/tick for testing/admin.

# # # Phase 5 additions (migration §9 L1259-1381):
# # #   - get_scheduler() → AsyncIOScheduler singleton with max_instances=1,
# # #     coalesce=True, interval=SCHEDULER_TICK_SECONDS (default 300s).
# # #   - run_tick(schema_name) → single-tenant tick with:
# # #       * status==Scheduled AND touchNumber<=6 filter (§9.4)
# # #       * zoneinfo business-hours filter (§9.2)
# # #       * PARTIAL throttle deterministic hash (§9.3)
# # #       * MailBridge dispatch via httpx 30s timeout (§9.5)
# # #   - run_tick_all_tenants() → iterates public.tenants ACTIVE rows,
# # #     per-tenant try/except (§9.6).
# # #   - _is_business_hours(now, tz_name) → §9.2 helper.
# # #   - _partial_throttle_passes(prospect_id, tick_bucket) → §9.3 helper.
# # #   - _send_via_mailbridge(db, config, sequence) → §9.5 helper.

# # # The SchedulerStatus row (id=1) is the persistent mirror of in-memory
# # # _STATUS; we update lastTickAt / sentSinceLastTick / skippedSinceLastTick /
# # # nextTickAt on every tick.
# # # """
...
# # # from app.models.phase3_models import SchedulerStatus
...
# # #     Per migration §9.2 L1288-1300. If `tz_name` is None or unparseable,
# # #     the input is treated as-is (callers should pass a tz-aware UTC datetime
# # #     so this default behaves reasonably for prospects with no timezone set).
# # #     """
# # #     if not tz_name:
# # #         local = now
# # #     else:
...
# # #         except Exception:  # noqa: BLE001 — unknown tz, fall back to UTC
# # #             local = now
...
# # #     payload = {
# # #         "from": config.fromEmail if config else "",
# # #         "from_name": (config.fromName if config else "") or "",
# # #         "to": recipient_email,
# # #         "subject": sequence.subjectLine or "",
# # #         "body": sequence.bodyCopy or "",
# # #         "provider": config.provider if config else "gmail",
...
# # #         if not msg_id:
# # #             raise RuntimeError("MailBridge response missing messageId")
...
# # #                 select(Sequence)
# # #                 .where(Sequence.status == EmailStatus.Scheduled)
...
# # #                     # Resolve the sequence owner (falls back to 'system' on legacy rows).
...
# # #                     # Use the sequence owner's config if available; fall back to
# # #                     # the tenant-level default config loaded above.
# # #                     if seq_owner and seq_owner != "system":
...
# # #                     msg_id = await _send_via_mailbridge(session, config, seq)
# # #                     seq.status = EmailStatus.Sent
# # #                     seq.sentAt = datetime.now(timezone.utc)
# # #                     seq.mailBridgeMessageId = msg_id
# # #                     sent += 1
...
# # #                         seq.status = EmailStatus.Sent
# # #                         seq.sentAt = datetime.now(timezone.utc)
# # #                         seq.mailBridgeMessageId = send_result.messageId
# # #                         sent += 1
# # #                     else:
# # #                         skipped += 1
...
# # #             status.lastTickAt = started
# # #             status.nextTickAt = started + timedelta(
# # #                 seconds=get_settings().SCHEDULER_TICK_SECONDS
# # #             )
# # #             status.sentSinceLastTick = sent
# # #             status.skippedSinceLastTick = skipped
# # #             await db.commit()
...
# # #         run = SchedulerRun(status="running")
# # #         db.add(run)
# # #         await db.commit()
# # #         # Re-fetch to get auto-generated ID (avoids DetachedInstanceError)
# # #         run = await db.get(SchedulerRun, run.id)
...
# #     # for this user, include it so MailBridge sends from the correct
# #     # connected mailbox.
...
#     # (2) the caller's Keycloak UUID (same value used during OAuth connect).
#     ext_user_id = (
