...
#         # asyncpg returns the connection to the pool on commit and strips any
#         # session-level SET commands including search_path. Re-set it now so
#         # all subsequent queries resolve to the correct tenant schema.
#         # We derive the tenant schema from the current search_path (set by
#         # get_db at request start) via SHOW search_path — if that also lost
#         # context, fall back to querying the Sequence table via public schema
#         # which won't work, so we must re-set it.
#         try:
#             _sp_row = (await db.execute(text("SHOW search_path"))).fetchone()
#             _sp = (_sp_row[0] if _sp_row else "") or ""
#             _tenant_schema = ""
#             for _tok in _sp.split(","):
#                 _tok = _tok.strip().strip('"')
#                 if _tok.startswith("tenant_"):
#                     _tenant_schema = _tok
#                     break
#             if _tenant_schema:
#                 await db.execute(text(f'SET search_path TO "{_tenant_schema}", public'))
#         except Exception:  # noqa: BLE001 — best-effort; queries may still work
#             pass

#         sent = 0
#         skipped = 0
#         try:
#             # ── Load Scheduled sequences, excluding suppressed prospects ───────
#             # JOIN with Prospect at the SQL level so unsubscribed prospects are
#             # filtered out in ONE query — no separate per-row check needed and
#             # no risk of search_path being lost between the sequence fetch and
#             # the suppression check.
#             _seq_rows = await db.execute(
#                 text(
#                     'SELECT s.id FROM "Sequence" s '
#                     'JOIN "Prospect" p ON p.id = s."prospectId" '
#                     "WHERE s.status = 'Scheduled' "
#                     '  AND p.suppressed IS NOT TRUE '
#                     "  AND p.consent_status <> 'withdrawn' "
#                     'ORDER BY s."createdAt" ASC '
#                     'LIMIT :lim'
#                 ),
#                 {"lim": max_send},
#             )
#             seq_ids = [r[0] for r in _seq_rows.fetchall()]
...
#             sequences = list(result.scalars().all())
#             logger.info(
#                 "scheduler.manual_tick.candidates",
#                 total_scheduled=len(seq_ids),
#                 after_suppression_filter=len(sequences),
#             )
#             for seq in sequences:
#                 # Phase 5 will add business-hours + throttle filters here.
#                 try:
