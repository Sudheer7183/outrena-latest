...
async def webhook(
    ...
    """Inbound tracking webhook — accepts both MailBridge native format and Outrena format.
    ...
    Detection: top-level "event_type" key → FORMAT 2, else FORMAT 1.
 
    FORMAT 2 event_type mapping:
        email.bounced      → bounced
        email.received     → replied
        email.delivered    → sent
        email.opened       → opened
        sequence.step_sent → sent
        (anything else)    → accepted silently, no DB change
 
    sequenceId resolution (FORMAT 2):
        MailBridge does not know Outrena's Sequence UUID. We resolve it by
        matching Sequence.mailBridgeMessageId == payload.message_id. If no
        match is found the event is logged and counted as rejected.
 
    HMAC verification:
        Dev mode → skipped. Prod with secret → X-MailBridge-Signature required.
        MailBridge signs as "sha256=<hex>" — the prefix is stripped before compare.
    """
    ...
    if "event_type" in body_dict:
        ...
        if outrena_event is None:
            # Unknown event type — ack so MailBridge doesn't retry endlessly
        ...
        # Resolve Outrena sequenceId via Sequence.mailBridgeMessageId
        outrena_sequence_id: str | None = None
        if message_id:
            seq_lookup = await db.execute(
                select(Sequence).where(Sequence.mailBridgeMessageId == message_id)
            )
        ...
        if outrena_sequence_id is None:
            ...
            ok = await _send_service.apply_tracking_event(db, tracking_event)
            if ok:
                accepted += 1
                _logger.info(
