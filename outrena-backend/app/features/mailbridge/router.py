...
# # ── BatchSend completion webhook ────────────────────────────────────────────
# def _verify_batch_hmac(signature_header: str, secret: str, raw_body: bytes) -> bool:
#     """Constant-time compare for the BatchSend completion webhook.

#     MailBridge's batch-send background task signs with the 'sha256=' prefix
#     convention (same as its other outbound webhooks) — the prefix is
#     stripped here before comparing, so this intentionally does NOT reuse
#     the existing bare-hex _verify_hmac above (that one backs the
#     pre-existing /mailbridge/webhook endpoint and is left untouched).
#     """
#     header = signature_header[7:] if signature_header.startswith("sha256=") else signature_header
#     expected = hmac.new(secret.encode(), raw_body, hashlib.sha256).hexdigest()
#     return hmac.compare_digest(header, expected)


# @_mailbridge_router.post("/batch-complete", response_model=BatchCompleteResponse)
# async def batch_complete(
#     request: Request,
#     db: AsyncSession = Depends(get_db),
# ) -> BatchCompleteResponse:
#     """BatchSend completion webhook from MailBridge POST /outbound/batch-send.

#       * HMAC-verified via X-MailBridge-Signature, secret from the active
#         MailBridgeConfig.webhookSecret (same resolution as the existing
#         /mailbridge/webhook endpoint). If webhookSecret is null, skip
#         verification and log a warning — same fail-open convention as the
#         existing webhook endpoint.
#       * Dev mode (ENVIRONMENT=development) skips HMAC, same as the
#         existing webhook endpoint.
#       * Delegates the per-row status transitions (with terminal-status
#         guard + quota release on failure) to
#         MailBridgeService.apply_batch_complete.

#     The raw body is read once and re-parsed into BatchCompleteWebhookPayload
#     so the HMAC is computed over the exact bytes MailBridge signed.
#     """
#     settings = get_settings()
#     raw_body = await request.body()
...
#         if secret:
#             signature = request.headers.get("X-MailBridge-Signature", "")
#             if not signature or not _verify_batch_hmac(signature, secret, raw_body):
#                 raise HTTPException(
#                     status.HTTP_401_UNAUTHORIZED,
#                     "Invalid webhook signature",
#                 )
#         else:
#             logger.warning("mailbridge.batch_complete.no_webhook_secret_configured")
#         # No secret configured → cannot enforce HMAC; fall through.

#     try:
#         payload = BatchCompleteWebhookPayload.model_validate_json(raw_body)
#     except Exception as exc:  # noqa: BLE001 — surface as 400 not 500
#         raise HTTPException(
#             status.HTTP_400_BAD_REQUEST,
#             f"Invalid batch-complete payload: {exc}",
#         ) from exc

#     accepted, rejected = await _send_service.apply_batch_complete(db, payload.results)
#     return BatchCompleteResponse(
#         batchId=payload.batchId, accepted=accepted, rejected=rejected
#     )
...
async def batch_complete(
    ...
    tenant_slug: str = Query(
        ...,
        description=(
            "Tenant slug for this batch — required because this endpoint "
            "is exempt from subdomain-based tenant resolution (see "
            "EXEMPT_PREFIXES in app/middleware/tenant_middleware.py). "
            "MailBridge is a separate service that may not be able to "
            "resolve/reach every tenant's subdomain, so the scheduler "
            "passes tenant_slug as a query parameter on the callbackUrl "
            "instead — same pattern as /api/v1/public/unsubscribe."
        ),
    ),
) -> BatchCompleteResponse:
    """BatchSend completion webhook from MailBridge POST /outbound/batch-send.
 
      * Tenant resolution: via `tenant_slug` query param, NOT the request's
        subdomain — this route is listed in TenantMiddleware.EXEMPT_PREFIXES,
        so `Depends(get_db)` would otherwise silently fall back to the
        'public' schema (see get_db's docstring) rather than the tenant's
        schema. A dedicated session is opened here instead, with
        `SET search_path` driven by tenant_slug, exactly like
        /api/v1/public/unsubscribe.
      * HMAC-verified via X-MailBridge-Signature, secret from the active
        MailBridgeConfig.webhookSecret (same resolution as the existing
        /mailbridge/webhook endpoint, now read from the tenant_slug-scoped
        session). If webhookSecret is null, skip verification and log a
        warning — same fail-open convention as the existing webhook
        endpoint.
      * Dev mode (ENVIRONMENT=development) skips HMAC, same as the
        existing webhook endpoint.
      * Delegates the per-row status transitions (with terminal-status
        ...
        try:
            ...
            if not settings.is_development:
                ...
                if secret:
                    ...
                    logger.warning(
                        "mailbridge.batch_complete.no_webhook_secret_configured",
                        tenant_slug=tenant_slug,
                    )
                # No secret configured → cannot enforce HMAC; fall through.
