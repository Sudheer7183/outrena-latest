...
#             # Raw text() status filter avoids the SAEnum cross-schema cast error.
#             # ORM or_() with Sequence.status == 'Bounced' generates a typed
#             # "email_status = $1::tenant_X.email_status" bind that fails with
#             # "operator does not exist: email_status = tenant_X.email_status"
#             # when asyncpg switches schemas. String literals in text() bypass
#             # the cast entirely.
#             # We also include status='Bounced' with bouncedAt=NULL — these were
#             # flipped by earlier code paths that didn't stamp the timestamp;
#             # the bounce scan backfills bouncedAt for them.
#             # All WHERE clauses use raw text() to avoid asyncpg prepared-statement
#             # cache errors across SET search_path schema switches. Any ORM expression
#             # with bind parameters (!=, ==, .in_) generates typed casts like
#             # $1::VARCHAR that cause InvalidCachedStatementError after a schema
#             # switch. String literals in text() bypass the cast entirely.
#             rows = (await db.execute(
#                 select(Sequence)
#                 .where(text(
#                     "(status = 'Sent' OR (status = 'Bounced' AND \"bouncedAt\" IS NULL))"
#                     " AND \"sentAt\" IS NOT NULL"
#                     " AND (sent_via_external_user_id IS NOT NULL"
#                     "      OR (owner_user_id IS NOT NULL AND owner_user_id != 'system'))"
#                 ))
#             )).scalars().all()
...
#     Uses text() to avoid asyncpg typed-bind cache errors on schema switches.
#     """
#     result = await db.execute(
#         select(Sequence).where(text(f"id = '{sequence_id}'"))
#     )
...
#     Uses raw SQL text() for the IN clause to avoid asyncpg's prepared-statement
#     cache going stale after SET search_path switches schema.  The ORM expression
#     Prospect.id.in_([...]) generates typed binds ($1::VARCHAR, ...) which
#     trigger InvalidCachedStatementError on schema switches — the same root cause
#     as the Sequence status query.  Parameterised IN with text() is not easily
#     composable in SQLAlchemy asyncpg, so we inline the IDs as string literals;
#     all values are internal UUIDs written by Outrena so there is no injection risk.
#     """
#     if not prospect_ids:
#         return {}
...
#         async with AsyncSessionLocal() as db:
#             await db.execute(text(f'SET search_path TO "{schema}", public'))
#             # Build the IN list from safe internal UUIDs — no user-supplied input.
#             id_list = ", ".join(f"'{pid}'" for pid in prospect_ids)
#             raw_rows = (await db.execute(
#                 text(f'SELECT id, email, anonymized FROM "Prospect" WHERE id IN ({id_list})')
#             )).fetchall()

#         for row in raw_rows:
#             prospect_id, raw_email, anonymized = row[0], row[1], row[2]
#             if anonymized:
#                 continue
#             if not raw_email:
#                 continue
...
#                 decrypted = raw_email
#             if decrypted:
#                 result_map[prospect_id] = decrypted.strip().lower()
#     except Exception as exc:  # noqa: BLE001
#         logger.warning(
...
async def _stamp_bounce(
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Suppress the prospect on bounce so they are excluded from ALL future
        # sends — multi-touch would otherwise create Touch 2/3/... for a prospect
        # whose email does not exist, wasting quota and inflating bounce counts.
        try:
            await db.execute(text(f'SET search_path TO "{schema}", public'))
            _seq_res = await db.execute(
                text('SELECT "prospectId" FROM "Sequence" WHERE id = :sid'),
                {"sid": sequence_id},
            )
            _seq_row = _seq_res.fetchone()
            if _seq_row and _seq_row[0]:
                await db.execute(
                    text(
                        "UPDATE \"Prospect\" "
                        "SET suppressed = TRUE, "
                        ...
                        "    \"suppressedAt\" = NOW() "
                        "WHERE id = :pid AND suppressed IS NOT TRUE"
                    ),
                    {"pid": _seq_row[0]},
                )
                await db.commit()
                logger.info(
                    "reply_poller.prospect_suppressed_on_bounce",
                    schema=schema,
                    sequence_id=sequence_id,
                    prospect_id=_seq_row[0],
                )
        except Exception as _exc:  # noqa: BLE001
            logger.warning(
                "reply_poller.prospect_suppression_failed",
                schema=schema,
                sequence_id=sequence_id,
                error=str(_exc),
            )
