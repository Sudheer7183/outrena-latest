...
class Sequence(Base, CuidPrimaryKey, TimestampMixin):
    ...
    bouncedAt: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    mailBridgeMessageId: Mapped[str | None] = mapped_column(String, nullable=True)
    # BatchSend — the MailBridge batchId this row was dispatched under while
    # status == 'BatchPending'. NULL for rows sent via the legacy per-row
    # path or not yet dispatched. Traceability only — the completion
    # webhook handler looks rows up by sequenceId, not this column. Added
    # by alembic/versions/0024_batch_send_status.py.
    mailBridgeBatchId: Mapped[str | None] = mapped_column(String, nullable=True)
    bounceReason: Mapped[str | None] = mapped_column(String, nullable=True)
