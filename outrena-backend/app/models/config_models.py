...
class MailBridgeConfig(Base, CuidPrimaryKey, TimestampMixin):
    ...
    # SupportTicket.created_by_user_id).
    owner_user_id: Mapped[str | None] = mapped_column(String(128), nullable=True)
    # MailBridge tenancy API key (mb_live_...) — used as Bearer token for
    # all MailBridge API calls in tenancy mode. Obtained from
    # POST /platform/register on the MailBridge instance.
    mailbridge_api_key: Mapped[str | None] = mapped_column(String, nullable=True)
    # The external_user_id mapping — when Outrena connects a user's mailbox
    # via MailBridge identity propagation (POST /connect/{provider}/start),
    # it passes the Outrena user's Keycloak UUID as external_user_id. This
    # column stores it so send calls can identify which MailBridge mail
    # account to use.
    mailbridge_external_user_id: Mapped[str | None] = mapped_column(String(128), nullable=True)
