...
class EmailStatus(str, enum.Enum):
    ...
    QaPassed = "QaPassed"
    Scheduled = "Scheduled"
    # BatchSend intermediate state: set the moment a sequence is folded
    # into a POST /outbound/batch-send call, replaced with Sent/Failed by
    # the batch completion webhook handler once MailBridge confirms the
    # individual result. Excluded from run_tick's/manual_tick's
    # `WHERE status = 'Scheduled'` candidate query by construction, so a
    # row in this state is never picked up twice. Requires the
    # 0024_batch_send_status migration to add this value to the per-tenant
    # Postgres `email_status` enum type.
