...
class Settings(BaseSettings):
    ...
    # ── BatchSend (scheduler batch dispatch to MailBridge /outbound/batch-send) ──
    # Single global switch — default False. Applies to both the automatic
    # scheduler tick (run_tick) and the manual "Run Tick"/"Trigger Now" UI
    # actions (SchedulerService.manual_tick) identically. When False, both
    # paths behave exactly as they did before BatchSend existed.
    BATCH_SEND_ENABLED: bool = True
    # Fallback callback URL used only when a tenant slug can't be resolved
    # (should not happen in normal multi-tenant operation — the scheduler
    # builds a per-tenant subdomain URL instead; see
    # app/features/scheduler/service.py::_build_batch_callback_url).
    OUTRENA_BATCH_CALLBACK_URL: str = "https://outrena.com/api/v1/mailbridge/batch-complete"
