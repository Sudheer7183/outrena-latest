// // /**
// //  * AuthContext.tsx — Keycloak JS adapter + React auth state.
// //  *
// //  * Replaces NextAuth's `SessionProvider` (migration §7.6). On mount it
// //  * initialises `keycloak-js` with PKCE, keeps the access token in sync with the
// //  * module-level holder used by `apiClient`, and exposes the decoded role +
// //  * tenant_slug claims via `useAuth()`.
// //  *
// //  * A `VITE_DEV_BYPASS_AUTH` flag enables a mock session for local development
// //  * without a running Keycloak realm (the SPA still renders + routes normally).
// //  */
...
// //         // Already tried re-login once this session — do not redirect again.
// //         // The user will see an error state on the page that triggered the 401.
// //         console.warn(
// //           "[AuthContext] 401 received after re-login attempt — " +
// //             "possible tenant_slug claim missing from Keycloak token or " +
// //             "TenantMiddleware cannot resolve tenant. Not redirecting again.",
// //         );
// //         return;
// //       }
...
//   // redirect comes back and API calls still return 401, stop redirecting and
//   // set an error state so the user sees a message instead of a blank loop.
//   // Circuit breaker key stored in sessionStorage so it survives Keycloak
//   // redirect cycles. useRef resets on every page load/redirect, which means
//   // the old ref-based circuit breaker never actually fired — every redirect
//   // back from Keycloak created a fresh component tree with loginAttempted=false,
//   // allowing the loop to continue indefinitely.
...
//       if (DEV_BYPASS) return;
//       if (loginAttemptedRef.current) {
//         // Already tried re-login once this browser session — stop redirecting.
//         // The persistent sessionStorage flag means this check survives the
//         // Keycloak redirect and actually breaks the loop.
//         console.warn(
//           "[AuthContext] 401 after re-login — tenant_slug missing from " +
//             "Keycloak token or TenantMiddleware cannot resolve tenant. " +
//             "Check that the user has a tenant_slug attribute in Keycloak " +
//             "and that the realm-export mapper is configured correctly. " +
//             "Not redirecting again this session.",
//         );
//         // Clear the flag so next manual login attempt works
...
//   // Clear the login-attempted flag on successful authentication so the user
//   // can log out and log back in without being stuck.
//   useEffect(() => {
//     if (user) {
//       sessionStorage.removeItem(LOGIN_ATTEMPTED_KEY);
