...
//   }
//   // When sending FormData (e.g. CSV file upload), axios must set the
//   // Content-Type to multipart/form-data with the correct boundary automatically.
//   // The default "application/json" header on the instance overrides that, which
//   // causes FastAPI to see no "file" field and return a validation_error.
//   // Deleting the header here lets axios detect FormData and set the right type.
...
//     /** When true, skips the CONFIGURE_TOTP Keycloak requiredAction so the
//      *  tenant admin can log in immediately with their password.
//      *  Defaults to true on the backend when temporary_password is set and
//      *  send_invitation is false. Pass explicitly to override. */
//     skip_mfa?: boolean;
//     integration_mode?: "tenant_managed" | "platform_managed";
//   }) =>
...
//       params,
//     ),
//   // Ad-hoc flow run — POST /flows/{flow_id}/run?icp_profile_id=…
//   runFlow: (flowId: string, icpProfileId: string, llmConfigId?: string) => {
//     const params = new URLSearchParams({ icp_profile_id: icpProfileId });
//     if (llmConfigId) params.set("llm_config_id", llmConfigId);
//     return http.post<{ run_id: string; status: string; message: string }>(
//       `/api/v1/flows/${flowId}/run?${params.toString()}`,
//       {},
//     );
//   },
// };
...
/** schedulerApi — scheduler status + manual tick + trigger + runs + new drill-down. */
export const schedulerApi = {
  status: () => http.get<SchedulerStatus>("/api/v1/scheduler/status"),
  tick: (body?: { tenantScoped?: boolean; maxSend?: number }) =>
    http.post<ManualTickResponse>("/api/v1/scheduler/tick", body ?? {}),
  ...
  /** GET /scheduler/campaign-schedules — sequences grouped by campaign */
  campaignSchedules: (params?: {
    campaign_id?: string;
    status?: string;
    limit?: number;
    offset?: number;
  }) =>
    http.get<{ items: CampaignScheduleItem[]; total: number }>(
      "/api/v1/scheduler/campaign-schedules",
      params,
    ),
  /** GET /scheduler/skipped-details — skip reason drill-down */
  skippedDetails: (params?: {
    run_id?: string;
    campaign_id?: string;
    skip_reason?: string;
    since?: string;
    ...
    http.get<SkipLogResponse>(
      "/api/v1/scheduler/skipped-details",
      params,
    ),
  ...
  dailySent: (params?: {
    ...
    http.get<{ items: DailySentItem[]; total: number }>(
      "/api/v1/scheduler/daily-sent",
      params,
    ),
};
