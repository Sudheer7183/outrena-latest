...
import {
  ...
  ShieldAlert, XCircle, Mail, User, ArrowDown, AlertTriangle,
} from 'lucide-react';
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="rounded-lg border bg-muted/30 p-3 text-sm">
          ...
          <p className="text-sm whitespace-pre-wrap leading-relaxed">
            ...
            {draft.sentEmailBody ? (
              (() => {
                const isHtml = /<[a-z][\s\S]*>/i.test(draft.sentEmailBody);
                return isHtml ? (
                  <div
                    className="text-sm leading-relaxed prose prose-sm max-w-none [&_a]:text-blue-600 [&_ul]:list-disc [&_ul]:ml-4 [&_ol]:list-decimal [&_ol]:ml-4 [&_strong]:font-semibold [&_p]:mb-2 [&_li]:mb-1"
                    dangerouslySetInnerHTML={{ __html: draft.sentEmailBody }}
                  />
                ) : (
                  <p className="text-sm whitespace-pre-wrap leading-relaxed">{draft.sentEmailBody}</p>
                );
              })()
            ) : (
...
export function ReplyInboxPage() {
  ...
  const { data: trackingSummary, isLoading: trackingLoading } = useQuery<TrackingSummary>({
    queryKey: ['tracking-summary', trackingDays],
    queryFn: () => http.get<TrackingSummary>('/api/v1/analytics/tracking-summary', { days: trackingDays }),
    staleTime: 60_000,
    retry: false,
  });
 
  const { data: rawBounced } = useQuery({
    queryKey: ['sequences-bounced'],
    queryFn: () => http.get<unknown>('/api/v1/sequences', { status: 'Bounced', limit: 200 }),
    staleTime: 60_000,
    retry: false,
  });
  const bouncedSequences = normalise<BouncedSequence>(rawBounced);
  ...
  const filteredDrafts = useMemo(() => {
  const list = showAutoPilotOnly ? replies.filter((r) => r.autoPilotEligible) : replies;
    ...
    <div className="space-y-4">
      ...
      {trackingLoading ? (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {[0,1,2].map(i => <div key={i} className="rounded-lg border p-4 animate-pulse bg-muted/30 h-20" />)}
        </div>
      ) : (
        <div className="space-y-3">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              { label: 'Sent', value: trackingSummary?.total_sent ?? 0, sub: `Last ${trackingDays} days`, color: 'text-blue-700', bg: 'bg-blue-50/50 border-blue-200', icon: <Mail className="h-4 w-4 text-blue-600" /> },
              { label: 'Replied', value: trackingSummary?.total_replied ?? 0, sub: trackingSummary ? `${(trackingSummary.reply_rate * 100).toFixed(1)}% rate` : '—', color: 'text-emerald-700', bg: 'bg-emerald-50/50 border-emerald-200', icon: <MessageCircleReply className="h-4 w-4 text-emerald-600" /> },
              { label: 'Bounced', value: trackingSummary?.total_bounced ?? 0, sub: trackingSummary ? `${(trackingSummary.bounce_rate * 100).toFixed(1)}% rate` : '—', color: (trackingSummary?.bounce_rate ?? 0) > 0.05 ? 'text-red-700' : 'text-orange-700', bg: (trackingSummary?.bounce_rate ?? 0) > 0.05 ? 'bg-red-50/50 border-red-200' : 'bg-orange-50/50 border-orange-200', icon: <AlertTriangle className="h-4 w-4 text-orange-500" /> },
            ].map(s => (
          ...
          {(trackingSummary?.total_bounced ?? 0) > 0 && (trackingSummary?.top_bounce_reasons.length ?? 0) > 0 && (
            <Card className="border-orange-200 bg-orange-50/30">
              <CardContent className="p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {trackingSummary!.top_bounce_reasons.map(r => (
                    <span key={r.reason} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-orange-100 text-orange-800 text-[11px] border border-orange-200">
          ...
          {(trackingSummary?.bounce_rate ?? 0) > 0.05 && (
            <div className="rounded-md border border-red-200 bg-red-50 p-3 flex items-start gap-2 text-xs text-red-800">
              <AlertTriangle className="h-3.5 w-3.5 mt-0.5 shrink-0" />
              <span><strong>High bounce rate ({((trackingSummary?.bounce_rate ?? 0) * 100).toFixed(1)}%)</strong> — benchmark is &lt;5%. Check the <strong>Bounced</strong> tab. High bounces hurt your sending reputation.</span>
            </div>
          )}
        </div>
      )}
      ...
      <Tabs defaultValue="manual" className="w-full">
        <TabsList className="grid w-full max-w-lg grid-cols-3">
          ...
          <TabsTrigger value="bounced">
            ...
            {bouncedSequences.length > 0 && (
              <Badge className="ml-1.5 bg-red-100 text-red-700 border-red-200 text-[10px] px-1 py-0 border">
                {bouncedSequences.length}
              </Badge>
            )}
          </TabsTrigger>
        </TabsList>
          ...
          {bouncedSequences.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Bounces are detected automatically by the reply poller every 2 minutes.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              <p className="text-xs text-muted-foreground">
                {bouncedSequences.length} bounced sequence{bouncedSequences.length === 1 ? '' : 's'} — these addresses could not be delivered to. Affected prospects are suppressed from future sends.
              </p>
              <div className="rounded-md border overflow-hidden">
                <table className="w-full text-xs">
                  <thead className="bg-muted/40 border-b">
                    <tr>
                      <th className="text-left p-3 font-medium text-muted-foreground">Touch</th>
                  ...
                  <tbody>
                    {bouncedSequences.map((seq, idx) => (
                      <tr key={seq.id} className={cn('border-b last:border-0', idx % 2 === 0 ? 'bg-background' : 'bg-muted/20')}>
                        <td className="p-3 font-medium">Touch {seq.touchNumber}</td>
                        <td className="p-3">
                          {seq.subjectLine
                            ? <span className="truncate max-w-[200px] block">{seq.subjectLine}</span>
                        ...
                        <td className="p-3 text-muted-foreground">
                          {seq.bouncedAt ? new Date(seq.bouncedAt).toLocaleString() : '—'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
