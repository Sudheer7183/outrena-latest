...
export function AnalyticsPage() {
  ...
  // NOTE: /api/v1/sequences has no `days` filter — returns the most recent
  // `limit` sequences regardless of date. These are used for:
  //  · Sequence Status Distribution (all-time status breakdown)
  //  · Sequence Step Performance (reply rate per touch number)
  //  · Intent Source Attribution (prospect intent → reply correlation)
  //  · QA Score Distribution
  // All four charts are explicitly labelled "from the most recent 500 sequences".
  const sequencesQ = useQuery({
  ...
  const campaignPerformance = useMemo(() => {
    ...
    >();
    for (const m of metrics) {
      const cur = byCampaign.get(m.campaignId) ?? { sent: 0, replied: 0, bounced: 0 };
      cur.sent += m.totalSent;
      cur.replied += m.totalReplied;
      cur.bounced += m.totalBounced;
      byCampaign.set(m.campaignId, cur);
  ...
  /* ── AN-3: Sequence Step Performance (touch 1-7) ── */
  const stepPerformance = useMemo(() => {
    const byTouch = new Map<number, { sent: number; replied: number }>();
    for (const s of sequences) {
      const cur = byTouch.get(s.touchNumber) ?? { sent: 0, replied: 0 };
      if (s.sentAt) cur.sent += 1;
      if (s.repliedAt) cur.replied += 1;
      byTouch.set(s.touchNumber, cur);
    }
        ...
        <>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Activity Over Time</CardTitle>
              <CardDescription>
                Sent, replied, and bounced counts for the selected range.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {timeSeriesQ.isLoading ? (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Aggregated send/reply/bounce metrics per campaign.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {campaignPerformance.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                {statusDistribution.length === 0 ? (
                  ...
                  <>
                    {/* FIX: Increased height + no inline slice labels to avoid overlap.
                        Legend rendered separately below the chart so it never
                        collides with pie slices regardless of how many statuses exist. */}
                    ...
                    {/* Custom legend below — wraps cleanly at any width */}
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Sequence Step Performance</CardTitle>
                <CardDescription className="text-xs">
                  Reply rate by touch number (T1-T7).
                </CardDescription>
              </CardHeader>
              <CardContent>
                {stepPerformance.length === 0 ? (
            ...
            <Card>
              ...
              <CardContent>
                {benchmarksQ.isLoading ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { label: "Reply Rate",  value: "5–15%",  note: "Cold outreach avg" },
                        { label: "Bounce Rate", value: "< 5%",   note: "Hard bounce target" },
                        { label: "Positive Reply", value: "2–5%", note: "Interested/meeting" },
                        { label: "QA Score",    value: "> 70",   note: "Minimum acceptable" },
                      ].map((b) => (
                        <div key={b.label} className="rounded-md border bg-muted/30 p-2 text-center">
                          <p className="text-sm font-bold">{b.value}</p>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {/* Filter out any open-rate benchmark entries that shouldn't be shown */}
                    {benchmarks
                      .filter((b) => !b.key?.toLowerCase().includes("open_rate") && !b.label?.toLowerCase().includes("open rate"))
                      .map((b) => (
                      <div key={b.key} className="rounded-md border p-2 text-center">
                        <p className="text-lg font-bold">
                          {b.value}
